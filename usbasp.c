...
static int usbasp_transmit(PROGRAMMER * pgm, unsigned char receive,
		unsigned char functionid, unsigned char send[4], unsigned char * buffer,
		int buffersize) {
	int nbytes;
#ifdef USE_LIBUSB_1_0
	...
	if (nbytes < 0) {
		fprintf(stderr, "%s: error: usbasp_transmit: %s\n", progname,
				strerror(libusb_to_errno(nbytes)));
		return -1;
	}
#else
	...
	for (j = 0; j < dev_list_len; ++j) {
		...
		if (descriptor.idVendor == vendor && descriptor.idProduct == product) {
			...
			if (r < 0) {
				...
				if (verbose > 1)
					fprintf(stderr, "%s: seen device from vendor ->%s<-\n",
							progname, string);
				if (strcmp(string, vendorName) == 0) {
					...
					if (r < 0) {
						...
						errorCode = USB_ERROR_NOTFOUND;
						if (verbose > 1)
							fprintf(stderr, "%s: seen product ->%s<-\n",
									progname, string);
...
static int usbasp_open(PROGRAMMER * pgm, char * port) {
	...
	} else if (usbOpenDevice(&PDATA(pgm)->usbhandle, USBASP_ZHIFENG_VID,
			"zhifengsoft", USBASP_ZHIFENG_PID, "USBHID") == 0) {
		if (PDATA(pgm)->usbhandle) {
			fprintf(stdout, "zhifen is ok\n");
		}
		fprintf(stdout, "Found zhifeng usbasp!maybe the port is unknown.\n",
				port);
	} else if (usbOpenDevice(&PDATA(pgm)->usbhandle, USBASP_SHARED_VID,
			"www.fischl.de",
			USBASP_SHARED_PID, "USBasp") != 0) {
		/* check if device with old VID/PID is available */
		if (usbOpenDevice(&PDATA(pgm)->usbhandle, USBASP_OLD_VID,
				"www.fischl.de",
...
static void usbasp_close(PROGRAMMER * pgm) {
	if (PDATA(pgm)->usbhandle != NULL) {
		...
		usbasp_transmit(pgm, 1, USBASP_FUNC_DISCONNECT, temp, temp,
				sizeof(temp));
#ifdef USE_LIBUSB_1_0
...
static int usbasp_cmd(PROGRAMMER * pgm, unsigned char cmd[4],
		unsigned char res[4]) {
	int nbytes = usbasp_transmit(pgm, 1, USBASP_FUNC_TRANSMIT, cmd, res,
			sizeof(res));
...
static int usbasp_program_enable(PROGRAMMER * pgm, AVRPART * p) {
	...
	int nbytes = usbasp_transmit(pgm, 1, USBASP_FUNC_ENABLEPROG, cmd, res,
			sizeof(res));
...
static int usbasp_paged_load(PROGRAMMER * pgm, AVRPART * p, AVRMEM * m,
	...
	while (wbytes) {
		...
		cmd[2] = address >> 16;
		cmd[3] = address >> 24;
		usbasp_transmit(pgm, 1, USBASP_FUNC_SETLONGADDRESS, cmd, temp,
				sizeof(temp));
...
static struct sckoptions_t usbaspSCKoptions[] = {
		{ USBASP_ISP_SCK_1500, 1500000 }, { USBASP_ISP_SCK_750, 750000 }, {
				USBASP_ISP_SCK_375, 375000 }, { USBASP_ISP_SCK_187_5, 187500 },
		{ USBASP_ISP_SCK_93_75, 93750 }, { USBASP_ISP_SCK_32, 32000 }, {
				USBASP_ISP_SCK_16, 16000 }, { USBASP_ISP_SCK_8, 8000 }, {
				USBASP_ISP_SCK_4, 4000 }, { USBASP_ISP_SCK_2, 2000 }, {
				USBASP_ISP_SCK_1, 1000 }, { USBASP_ISP_SCK_0_5, 500 } };
...
static int usbasp_set_sck_period(PROGRAMMER *pgm, double sckperiod) {
	...
	if (sckperiod == 0) {
		...
		if (verbose >= 1)
			fprintf(stderr,
					"%s: auto set sck period (because given equals null)\n",
					progname);
		...
		if (sckfreq >= usbaspSCKoptions[0].frequency) {
			...
			int i;
			for (i = 0;
					i < sizeof(usbaspSCKoptions) / sizeof(usbaspSCKoptions[0]);
					i++) {
	...
	int nbytes = usbasp_transmit(pgm, 1, USBASP_FUNC_SETISPSCK, cmd, res,
			sizeof(res));

	if ((nbytes != 1) | (res[0] != 0)) {
		fprintf(stderr,
				"%s: warning: cannot set sck period. please check for usbasp firmware update.\n",
				progname);
		return -1;
	}
